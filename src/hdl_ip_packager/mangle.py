"""Automatic SystemVerilog **package** name-mangling for multi-version coexistence.

When the resolver keeps two versions of one package (the ``isolate_namespaces``
conflict policy), they collide at elaboration: SystemVerilog puts every ``package``
name in one global namespace, so two ``package bus_pkg;`` declarations clash. This
module rewrites each version's package name to a version-unique one
(``bus_pkg`` -> ``bus_pkg__v1_1_0`` / ``bus_pkg__v2_0_0``) and rewrites every
consumer's references to the version *that consumer resolved to*, so both can build.

Scope and safety: only SystemVerilog/Verilog **packages** are handled, because the
package-reference contexts -- ``package <name>``, ``endpackage : <name>``,
``import <name>::``, and ``<name>::`` scoped references -- are syntactically
unambiguous, so a comment/string-aware lexer can rewrite them safely without a full
parser. The lexer skips comments and string literals. **Not** handled (and refused
upstream): two versions of a *module*/*interface* (instantiation position is
ambiguous without parsing), VHDL, and macros that *construct* a package name by
token pasting -- such a macro body is left untouched and will not be mangled.
...
def mangled_name(name: str, version: AnyVersion) -> str:
    """Return *name* suffixed with a version-unique, SystemVerilog-safe tag.

    ``mangled_name("bus_pkg", Version(1, 1, 0))`` -> ``"bus_pkg__v1_1_0"``.
    """
...
def rewrite_sv_packages(source: str, renames: Mapping[str, str]) -> str:
    """Rewrite package declarations and references in *source* per *renames*.
    ...
    for index, (kind, text) in enumerate(tokens):
        if kind == "ident" and text in renames and _is_package_position(sig, position_of[index]):
            out.append(renames[text])
        else:
            out.append(text)
    return "".join(out)


def _is_package_position(sig: Sequence[tuple[int, str, str]], position: int) -> bool:
    """True if the significant token at *position* is a package declaration/reference."""
...
class GenSourceFile:
    """One source file taking part in generation: its key, text, and HDL kind."""

    key: tuple[str, str]  # (owning VLNV string, fileset-relative path) -- a stable id
    text: str
    is_systemverilog: bool  # SystemVerilog/Verilog (mangleable) vs VHDL/other
...
def plan_package_mangling(cores: Sequence[GenCore]) -> ManglePlan:
    ...
    be mangled safely -- two versions of a *module*/interface, or a non-SystemVerilog
    source in a conflicted core.
    """
    ...
    for ref, group in conflicted.items():
        ...
        for core in group:
            names = {n for f in core.files for n in declared_packages(f.text)}
    ...
    for core in cores:
        ...
        for source in core.files:
            rewritten[source.key] = rewrite_sv_packages(source.text, rename_map)
    return ManglePlan(
        rewritten=rewritten,
...
def _reject_unmangleable(ref: PackageRef, group: Sequence[GenCore]) -> None:
    """Refuse a conflict the package mangler cannot handle: VHDL, or module collisions."""
    if any(not f.is_systemverilog for core in group for f in core.files):
        raise BackendError(
            f"Cannot coexist two versions of {ref}: it has non-SystemVerilog sources, and "
            f"automatic name-mangling is only implemented for SystemVerilog packages. "
            f"Resolve to a single version (e.g. [resolution] on-conflict = 'use_latest')."
        )
    module_counts = Counter(
        name for core in group for f in core.files for name in declared_modules(f.text)
    )
    clashing = sorted(name for name, count in module_counts.items() if count >= 2)
    if clashing:
        raise BackendError(
            f"Cannot coexist two versions of {ref}: they declare colliding module/interface "
            f"name(s) {clashing}, and automatic name-mangling is only implemented for "
            f"SystemVerilog packages. Resolve to a single version or split the build."
        )
