...
| Multi-version coexistence for *modules*/*entities* (beyond packages) | `mangle.py`, `cli.py` | **In progress for 0.12.0** — design accepted (approach A, in-house scoped rewriter; SV modules/interfaces/programs + VHDL entities; generate-nested instantiations in scope; classify-all-or-refuse safety). See [docs/design/module-entity-coexistence.md](design/module-entity-coexistence.md); landing in small staged commits. _Progress:_ kind-aware mangler refactor + **SV module/program coexistence** (instantiation incl. parameter maps, instance arrays, multiple instances, generate-nested) + **VHDL entity coexistence** (entity/architecture/component declarations, direct `entity work.X` and component instantiation, generate-nested) are in, with classify-all-or-refuse (SV) + cross-ref guard. **SV interfaces** next, then integration + a demo build-lane proof. _Original analysis (June 2026), retained for context:_ **keep refusing was the prior stance, now superseded.** Package coexistence is done for SV and VHDL (`gen` name-mangles under `isolate_namespaces`); two versions of a SV *module*/interface or a VHDL *entity* are still **refused** — and that refusal is now the *intended* behavior, with a sharpened error that explains why and what to do (`_reject_unmangleable`). Feasibility analysis behind the decision: a package reference (`::` / `use work.`) sits in an unambiguous position, so it can be token-rewritten safely. An **SV module instantiation** (`foo u_foo (...)`) has no leading keyword and is indistinguishable from other constructs by token scan — it genuinely needs a full HDL parser (the parked "source-unit tokenizing" backlog item); rewriting it heuristically could silently corrupt a design. A **VHDL entity** is partly feasible parser-free: the direct-instantiation form (`label : entity work.foo`) is unambiguous like `use work.`, so entity/architecture/`end` labels + `entity work.X` could be rewritten, refusing when the entity is reached via an ambiguous *component* instantiation. That VHDL subset is a viable future increment, but was **consciously deferred** here (it would not cover the SV case and adds a refuse-or-rewrite branch). Revisit when the parser frontend lands (then both languages, fully) or if the VHDL-only subset becomes worth shipping on its own. |
