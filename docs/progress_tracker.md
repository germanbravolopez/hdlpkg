...
| Publish + discontinue `hdl-ip-packager` on PyPI | `shim/` | The metadata-only deprecation shim (`shim/pyproject.toml`, `hdl-ip-packager` **0.12.1**, depends on `hdlpkg>=0.13.0`, no code) was **built + metadata-verified** but **not yet published** — PyPI's latest `hdl-ip-packager` is still the real **0.12.0**, so `pip install hdl-ip-packager --upgrade` silently keeps users on old code. Publishing 0.12.1 makes `--upgrade` pull in `hdlpkg` and shows the deprecation on the PyPI page (the only nudge pip allows — there is no install-time message). One-time `twine upload` under the `hdl-ip-packager` project's own PyPI credentials (our OIDC `release.yml` only publishes `hdlpkg`); awaiting the maintainer's upload. Do **not** yank the pre-rename 0.1.0–0.12.0 releases (breaks reproducible pins). |
